...
class sfd:
    ...
    def fit(self, lambda_="auto", theta=0.5, max_iter=500, accuracy_threshold=1e-3, method="APGD"):
        ...
        if method == "APGD":
            ...
            l22_loss = SquaredL2Loss(dim=self.data.true_dim, data=self.data.data[self.data.mask]) * self.F
            pds = APGD(dim=self.dim, F=l22_loss, G=self.G, verbose=1, acceleration="CD",
            ...
            if "trend" in self.code:
                if "seas" in self.code:
                    self.estimate_seas = estimate["iterand"][0:(self.N_seas * self.M_seas)] \
                        .reshape(self.M_seas, self.N_seas).transpose()
                    self.estimate_trend = estimate["iterand"][(self.N_seas * self.M_seas):] \
                        .reshape(self.M_trend, self.N_trend).transpose()
                else:
                    self.estimate_trend = estimate["iterand"].reshape(self.M_trend, self.N_trend).transpose()

            else:
                self.estimate_seas = estimate["iterand"].reshape(self.M_seas, self.N_seas).transpose()
        ...
        if method == "PDS":
            ...
            l22_loss = SquaredL2Loss(dim=self.data.true_dim, data=self.data.data[self.data.mask]) * self.F
            pds = PDS(dim=self.dim, F=l22_loss, G=self.G, H=self.H, K=self.K, verbose=1, max_iter=max_iter,
            ...
            if "trend" in self.code:
                if "seas" in self.code:
                    self.estimate_seas = estimate["primal_variable"][0:(self.N_seas * self.M_seas)] \
                        .reshape(self.M_seas, self.N_seas).transpose()
                    self.estimate_trend = estimate["primal_variable"][(self.N_seas * self.M_seas):] \
                        .reshape(self.M_trend, self.N_trend).transpose()
                else:
                    self.estimate_trend = estimate["primal_variable"].reshape(self.M_trend, self.N_trend).transpose()

            else:
                self.estimate_seas = estimate["primal_variable"].reshape(self.M_seas, self.N_seas).transpose()
    ...
    def save_frames(self, t, code):

        for i in range(t.shape[0]):
            self.spatial_plot(t[i], code=code, mode="save", filename="frame" + str(i) + ".png")
            print("Frame " + str(i) + "/" + str(t.shape[0]) + " saved.")
    ...
    def mean(self, xyz, t, code):

        if xyz.ndim == 1:
            mean = np.mean(self.evaluation(xyz, t, code=code))
        else:
            mean = np.zeros(xyz.shape[0])
            for i in range(t.shape[0]):
                mean += self.evaluation(xyz, 0, code=code).reshape(-1)
            mean /= t.shape[0]
